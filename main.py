...
try:
        ...
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as file:
                try:
                    ...
                    return {
                        w: d for w, d in data.items()
                        if current_time - d.get("deleted_timestamp", current_time) <= 2592000
                    }
                except Exception:
                    pass
        return {}
    ...
    def create_empty_state(text, icon="ghost-outline"):
        ...
        icon_btn = MDIconButton(icon=icon, pos_hint={"center_x": .5}, theme_text_color="Hint")
    ...
    class ViewDictionaryScreen(MDScreen):
            ...
            self.dialog = MDDialog(
                ...
                buttons=[
                    ...
                    MDRaisedButton(
                        ...
                        on_release=lambda x: self.execute_delete(word_to_delete)
                    )
                ],
            )
            self.dialog.open()
        ...
        def refresh_list(self, *args):
            if not hasattr(self, 'ids') or 'words_container' not in self.ids:
                return
            self.ids.words_container.clear_widgets()
            words = load_words()

            if not words:
                self.ids.words_container.add_widget(
                    create_empty_state("Your dictionary is empty!\nClick + ADD to begin.", "book-open-blank-variant")
                )
                return
            ...
            for w, d in words.items():
                if search_query and search_query not in w:
                    continue
                if self.active_filter == "Struggling" and d.get('mastery', 0) >= 3:
                    continue
                elif self.active_filter not in ["All", "Struggling"] and \
                        self.active_filter.lower() not in d.get('category', '').lower():
                    continue
                filtered[w] = d

            if not filtered:
                self.ids.words_container.add_widget(
                    create_empty_state("No words match your filter.", "file-search-outline")
                )
                return

            sorted_items = (
                sorted(filtered.items(), key=lambda i: i[0])
                if sort_mode == "Alphabetical"
                else sorted(filtered.items(), key=lambda i: i[1].get('timestamp', 0), reverse=True)
            )
    ...
    class RecycleBinScreen(MDScreen):
        ...
        def refresh_bin(self):
            if not hasattr(self, 'ids') or 'bin_container' not in self.ids:
                return
            self.ids.bin_container.clear_widgets()
            bin_data = load_recycle_bin()

            if not bin_data:
                self.ids.bin_container.add_widget(
                    create_empty_state("Recycle Bin is empty.", "delete-empty-outline")
                )
                return
        ...
        def export_csv(self):
            ...
            try:
                with open(export_path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for w, d in words.items():
                        writer.writerow([w, d.get('category', ''), d.get('meaning', ''),
                                         d.get('example', ''), d.get('mastery', 0)])
    ...
    class FlashcardScreen(MDScreen):
        ...
        def show_current_word(self):
            ...
            if self.current_idx >= len(self.test_words):
                ...
                self.ids.test_meaning_label.text = "Session Finished!"
                self.ids.reveal_btn.disabled = True
                return
        ...
        def play_audio(self):
            if self.test_words and self.current_idx < len(self.test_words):
                speak(self.test_words[self.current_idx][0])
    ...
    class MCQScreen(MDScreen):
        ...
        def next_question(self):
            ...
            self.ids.feedback_label.text = "Choose the correct meaning:"
            ...
            for i, btn in enumerate(buttons):
                if i < len(self.options):
                    ...
                    btn.disabled = False
                    btn.opacity = 1
                    btn.background_color = MDApp.get_running_app().theme_cls.primary_color
                else:
                    btn.disabled = True
                    btn.opacity = 0
        ...
        def check_answer(self, selected_button):
            ...
            if selected_button.text == correct_meaning:
                ...
                selected_button.background_color = [0.2, 0.8, 0.2, 1]
                self.all_words[self.current_word]['mastery'] = \
                    self.all_words[self.current_word].get('mastery', 0) + 1
            else:
                self.ids.feedback_label.text = "INCORRECT! -1 Mastery"
                selected_button.background_color = [0.8, 0.2, 0.2, 1]
                for btn in buttons:
    ...
    class MyWordApp(MDApp):
        ...
        def change_theme(self, theme_name):
            themes = {
                "White (Sadabahar)": ("Light", "Blue"),
                "Dark": ("Dark", "DeepPurple"),
                "Police": ("Dark", "Indigo"),
                "Cool & Composed": ("Light", "Teal"),
                "R E A L E L": ("Dark", "Orange"),
            }
            style, palette = themes.get(theme_name, ("Dark", "Indigo"))
        ...
        def hook_keyboard(self, window, key, *args):
            if key == 27:
                if self.root.current == 'view':
                    return False
                elif self.root.current in ['flashcard', 'mcq_test']:
                    self.change_screen('test_menu', direction='right')
                else:
                    self.change_screen('view', direction='right')
                return True
            return False
    ...
    class CrashApp(App):
        def build(self):
            Window.clearcolor = (0.6, 0.1, 0.1, 1)
            sv = ScrollView()
            lbl = Label(
                text=error_message,
                size_hint_y=None,
